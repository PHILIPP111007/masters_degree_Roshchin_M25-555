...
$$
>
Пусть \(m(n)\) — минимальное расстояние до ближайшего целого куба для числа \(n\), вычисленное по формуле выше.

Тогда для любого нечётного целого числа \(n > 2\) выполняется следующее эмпирическое правило:
...
**Примечание:** В случае \(m(n) < 0.40 \cdot n\) число не отбрасывается «навсегда», а направляется на классическую проверку перебором делителей, что гарантирует **100% точность** и **отсутствие ложных срабатываний**.
